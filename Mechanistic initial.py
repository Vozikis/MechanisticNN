...
base_directory = "Videos/non-holonomic_pendulum/30_10_2024/fps_30"
num_trajectories = 11
...
def apply_filter(t_data, x_data, kernel_size=5):
    """
    Applies a median filter to smooth trajectory data.
    
    Args:
        t_data (torch.Tensor): Time data as a torch tensor.
        x_data (torch.Tensor): Trajectory data as a torch tensor.
        kernel_size (int): Size of the median filter kernel (must be odd).

    Returns:
        torch.Tensor: Filtered trajectory data.
    """
    # Convert torch tensors to numpy for filtering
...
def train():
    ...
    for i, trajectory in enumerate(all_trajectories):
        print(f"Applying filter to trajectory {i + 1}")
        t_data = torch.arange(len(trajectory), dtype=torch.float32).reshape(-1, 1)
        x_data = torch.tensor(trajectory, dtype=torch.float32).reshape(-1, 1)

        # Apply the filter
        t_data_filtered, x_data_filtered = apply_filter(t_data, x_data, kernel_size=5)

        # Replace the trajectory with the filtered one
        all_trajectories[i] = x_data_filtered.flatten().numpy()

        print(f"Training on trajectory {i + 1}/{len(all_trajectories)}...")

        dataset = RealDataset(trajectory_data=all_trajectories[i])
